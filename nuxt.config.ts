...
export default defineNuxtConfig({
  ...
  nitro: {
    // Empacota server/graphql/schema/*.graphql e app/components/playground/**
    // no build de produção (readFileSync/import.meta.glob direto não é
    // garantido pelo bundler do Nitro). Caminho absoluto: o `dir` de
    // serverAssets é resolvido a partir do srcDir do Nitro (já é `server/`),
    // não da raiz do projeto.
    serverAssets: [
      { baseName: 'graphql', dir: fileURLToPath(new URL('./server/graphql/schema', import.meta.url)) },
      { baseName: 'playground', dir: fileURLToPath(new URL('./app/components/playground', import.meta.url)) }
    ]
  }
})